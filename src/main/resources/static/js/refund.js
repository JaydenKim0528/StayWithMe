...
function requestRefund() {
    ...
    if (confirm("정말로 환불을 진행하시겠습니까?")) {
        const impUid = document.getElementById("uid").value; // 실제 imp_uid 값으로 대체하세요
        const cancelAmount = document.getElementById("amount").value; // 실제 imp_uid 값으로 대체하세요
        const cancelBy = document.getElementById("cancelBy").value; // 실제 imp_uid 값으로 대체하세요
        const bookingNo = document.getElementById("bookingNo").value; // 실제 imp_uid 값으로 대체하세요
