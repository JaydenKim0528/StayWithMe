...
//  List<ListDTO> getPlace(@Param("pi") PageInfoDTO pi);
  ...
  // 기간 게시물 수 조회
  int DateListCount(@Param("mainDTO") MainSearchDTO mainDTO);
  // 기간 게시물 조회
  List<ListDTO> dateSearch(@Param("mainDTO") MainSearchDTO mainDTO);
  // 기간 부가시설 조회
  List<String> dateFacil(@Param("mainDTO") MainSearchDTO mainDTO);
